...
class MessageHandler {
  ...
  handle(clientId, ws, data) {
    ...
    if (!partnerWs) {
      this._send(ws, { type: 'error', clientId, targetId: partner || '', message: '404' })
      return
    }

    switch (type) {
      case 1: // 强度减少
        this._sendToApp(partnerWs, clientId, partner, `strength-${channel}+0+1`)
        break

      case 2: // 强度增加
        this._sendToApp(partnerWs, clientId, partner, `strength-${channel}+1+1`)
        break

      case 3: // 强度设为指定值
        if (strength === undefined) {
          this._send(ws, { type: 'error', clientId, targetId: partner, message: '406' })
          return
        }
        this._sendToApp(partnerWs, clientId, partner, `strength-${channel}+2+${strength}`)
        break
  ...
  _handleBind(clientId, ws, data) {
    const { clientId: reqClientId, targetId } = data

    // 验证目标是否存在
    if (!this.cm.exists(targetId)) {
      this._send(ws, { type: 'bind', clientId, targetId, message: '401' })
      return
    }
  ...
  _handleWave(clientId, ws, partnerWs, partnerId, data) {
    ...
    try {
      ...
      const jsonPart = message.includes(':') ? message.split(':').slice(1).join(':') : message
